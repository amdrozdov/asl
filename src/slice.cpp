...
// codecs spec: https://www.signalogic.com/index.pl?page=audio_waveform_file_formats#table


void AudioSlicer::load_formats(){
	this->format_mapping = map<short int, string>();
...
void AudioSlicer::read(){
	...
	this->num_samples = double(header.Subchunk2Size) / double((header.NumOfChan) * bytes_per_sample);
	this->format_prefix = "Unknown";
	if(this->format_mapping.find(header.AudioFormat) != this->format_mapping.end()){
		this->format_prefix = this->format_mapping[header.AudioFormat];
	}
	this->duration = this->num_samples / double(this->header.SamplesPerSec);

	this->channels = vector<char*>();
	map<int,int> ch_pos = map<int,int>();
	for(int i=0;i<this->header.NumOfChan;i++){
		this->channels.push_back((char*)malloc(this->num_samples * bytes_per_sample));
		ch_pos[i] = 0;
	}
	char *buf = (char*)malloc(this->header.Subchunk2Size);
...
void AudioSlicer::extract_audio(chunk slice){
	...
	char * data = (char*)malloc(total_bytes);
	int p = 0;
	...
	free(data);
	if(this->is_verbose){
		cout << "Extracted interval [" << slice.sec_start<<":" << slice.sec_end << "] into '" << slice.filename << "'" << endl;
	}
}
...
void AudioSlicer::split_channels(string out_prefix){
	...
	for(int i=0;i<this->channels.size();i++){
		...
		fclose(wavFile);
		if(this->is_verbose){
			cout << "Extracted channel " << i << " into '" << out_prefix << i << ".wav'" << endl;
		}
	}
}
...
void AudioSlicer::slice(vector<chunk> chunks){
	for(int i=0;i<chunks.size();i++){
		...
		assert(chunks[i].sec_end <= int(this->duration) + 1);
